...
class UserProfilePasswordController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($customer) {
            return back()->with('success', 'Your profile has been successfully changed');
        } else {
            return back()->with('errors', 'Your profile failed to change');
        }
    }
        ...
        if ($customer_pass) {
            if (Hash::check($request->old_pass, $customer_pass->password)) {
                if ($request->new_pass == $request->confirm_pass) {
                    ...
                    return back()->with('erros', 'Your Password failed to change');
                }
            }
        }
    }
