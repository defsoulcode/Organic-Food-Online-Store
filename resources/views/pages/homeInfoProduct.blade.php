...
<div class="row mt-3">
    <h2 class="text-center font-semibold mt-5">Food Information</h2>
        ...
        <div class="card border-0 shadow-lg mt-4 mb-3" style="max-width: 600px;">
            <div class="row g-0">
                <div class="col-md-5 text-center shadow-inner m-auto">
                    @if ($info->image)
                        <img src="{{ asset('storage/' . $info->image) }}" class="img-fluid rounded" style="width: 100%; height: 100%; object-fit: cover;"/>
                    @else
                ...
                <div class="col-md-7">
                    <div class="card-body">
                        <h6 class="card-title font-weight-bolder text-center text-danger">{{ $info->name_prod }}</h4>
                        <p class="card-text font-semibold">Category: {{ $info->category->name }}</p>
                        <p class="card-text font-semibold">Distributor: {{ $info->distribut->name_distr }}</p>
                        <p class="card-text font-semibold">Code Product: {{ $info->code_prod }}</p>
                        <p class="card-text font-semibold">Stock: {{ $info->stock }} </p>
                        <p class="card-text font-semibold">Weight: {{ $info->weight / 1000}} Kg</p>
                        <p class="card-text font-semibold">Price: @currency($info->price)</p>
                        <div class="row">
                            <div class="col-md-4">
                                <form action="{{ route('cart.store') }}" method="POST" class="d-inline">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $info->id }}" />
                                    <input type="number" name="quantity" min="0" max="{{ $info->stock }}" 
                                        class="form-control text-center" value="{{ ($info->stock > 0) ? 1 : 0 }}" />
                            </div>
                            <div class="col-md-6">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Rating This Product </h5>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('review') }}">
                    @csrf
                    <input type="hidden" value="{{ $info->id }}" name="id_product">
