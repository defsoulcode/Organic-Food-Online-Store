    ...
    <div class="col-md-10">
        <div class="card border-0 shadow-lg my-5">
            <div class="card-header text-center">
                <h1 class="h2 text-gray-800">Edit Product</h1>
            </div>
            <div class="card-body">
                <form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name_prod') is-invalid @enderror"
                            id="name_prod" name="name_prod" required
                            value="{{ old('name_prod', $product->name_prod) }}">
                        @error('name_prod')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" name="distribut_id" id="distribut">
                            <option value="" disabled selected>Select Distribut </option>
                            @foreach ($distributs as $distribut)
                                <option value="{{ old('distribut_id', $distribut->id) }}" @if ($distribut->id == $product->distribut_id)
                                    selected
                                    @endif>
                                    {{ $distribut->name_distr }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="hidden" name="oldCover" value="{{ $product->image }}">
                        @if ($product->image)
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" class="form-control @error('detail') is-invalid @enderror" id="detail"
                            name="detail" required value="{{ old('detail', $product->detail) }}">
                        @error('detail')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
